...
describe('detectNonEnglishText', () => {
  ...
  test('respects custom englishThreshold option', () => {
    expect(detectNonEnglishText('The quick brown fox jumps', { englishThreshold: 0.9 })).toBe(
      false,
    );
  });

  test('respects minWordLength option', () => {
    expect(detectNonEnglishText('I am a big fan of this project', { minWordLength: 1 })).toBe(
      false,
    );
  });
...
describe('word-level detection', () => {
  ...
  test('handles text with document patterns mixed with English', () => {
    expect(detectNonEnglishText('The AEM01-WI-DSU06-SD01 document is ready for review')).toBe(
      false,
    );
  });
  ...
  test('franc cache eviction works when limit exceeded', () => {
    for (let i = 0; i < 5; i++) {
      detectNonEnglishText(
        `This is test sentence number ${i} for checking cache behavior in the system`,
      );
    }
  ...
  test('word cache eviction works when limit exceeded', () => {
    const longText =
      'The quick brown fox jumps over the lazy dog and runs through the field with great speed and energy';
